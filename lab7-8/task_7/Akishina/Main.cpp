...
// обработчик реации на клик мышки
...
int main() {
	...
	// отключение курсора
	glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
	// назначение обработчика прокрутки курсора
	glfwSetScrollCallback(window, scroll_callback);
	// назначение обработчика реакции на клик мыши
	...
	while (!glfwWindowShouldClose(window)) { // пока окно window не должно закрыться
		...
		for (int k = 0; k < models.size(); k++) {
			...
			for (int i = 0; i < figure.size(); i++) { // цикл по конечным точкам (от единицы)
