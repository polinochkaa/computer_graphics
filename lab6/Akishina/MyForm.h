...
#pragma endregion
	private: float left = 30, right = 100, top = 20, bottom = 50; //расстояние до границ окна
		   float minX = left, maxX; //диапазон измененния координат x
		   float minY = top, maxY;	//диапазон измененния координат y
		   float Wcx = left, Wcy;	//координаты левого нижненого угла прямоугольника
		   float Wx, Wy;	//ширина и высота прямоугольника
	private:System::Void rectCalc() {
		maxX = ClientRectangle.Width - right;	//диапазон измененния координат x
		maxY = ClientRectangle.Height - bottom;	//диапазон измененния координат y
		Wcy = maxY;	//координаты левого нижненого угла прямоугольника
		Wx = maxX - left;	//ширина прямоугольника
		Wy = maxY - top;	//высота и ширина прямоугольника
	}
	...
	private: System::Void MyForm_KeyDown(System::Object^ sender, System::Windows::Forms::KeyEventArgs^ e) {
		...
		case Keys::F: {
			if (Control::ModifierKeys == Keys::Shift) {
				...
				mat4 M =
					rotate(0.1, vec3(0, 1, 0)); // матрица вращения относительно Ox
				vec3 u_new = mat3(M) * vec3(0, 1, 0); // вращениенаправления вверх
				vec3 P_new = normalize(M * vec4(0, 0, -1, 1)); // вращение точки, в которую смотрит наблюдатель
				T = lookAt(vec3(0, 0, 0), P_new, u_new) * T;
			}
			break;
